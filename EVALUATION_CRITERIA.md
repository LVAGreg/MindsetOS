...
Each anti-pattern has a metadata comment:
`<!-- hits:N | last_caught:Loop-N | class:CLASS | severity:SEVERITY | blocker:yes/no -->`

- **hits**: times caught across all loops — higher = more likely to recur
- **last_caught**: most recent loop where it appeared
- **class**: category for filtering (token-violation, cross-file-mismatch, silent-failure, security, integration-gap, typescript, accessibility, mobile, state-management, design-duplication, analytics, webgl)
- **severity**: correctness | craft | security | functionality
- **blocker**: yes = blocks ship regardless of score; no = caps a score criterion

### For ERIL agents
Before implementing, run a mental filter: which classes apply to my task?
- Building a component that calls an API? → check `cross-file-mismatch` patterns
- Building UI with colors? → check `token-violation` patterns
- Building interactive elements? → check `accessibility` + `mobile` patterns
- Handling async operations? → check `silent-failure` + `state-management` patterns

### For V5 reviewer
On every bug caught, increment `hits` and update `last_caught` in the relevant entry.
If a new pattern emerges that isn't covered, append it with `hits:1`.

### Confidence tiers
- hits ≥ 5: **High-confidence** — assume it will recur. Hard-check in every relevant loop.
- hits 2–4: **Medium-confidence** — likely to recur. Check when class matches.
- hits 1: **Low-confidence** — catalogued once. Verify if class matches task.
...
<!-- hits:3 | last_caught:Loop-62 | class:state-management | severity:correctness | blocker:no -->
...
<!-- hits:3 | last_caught:Loop-62 | class:cross-file-mismatch | severity:correctness | blocker:yes -->
...
<!-- hits:2 | last_caught:Loop-62 | class:design-duplication | severity:craft | blocker:no -->
...
<!-- hits:2 | last_caught:Loop-62 | class:state-management | severity:correctness | blocker:no -->
...
<!-- hits:6 | last_caught:Loop-72 | class:silent-failure | severity:functionality | blocker:no -->
...
<!-- hits:1 | last_caught:Loop-62 | class:webgl | severity:correctness | blocker:no -->
...
<!-- hits:3 | last_caught:Loop-62 | class:integration-gap | severity:correctness | blocker:yes -->
...
<!-- hits:2 | last_caught:Loop-62 | class:cross-file-mismatch | severity:correctness | blocker:yes -->
...
<!-- hits:1 | last_caught:Loop-62 | class:cross-file-mismatch | severity:functionality | blocker:yes -->
...
<!-- hits:1 | last_caught:Loop-62 | class:mobile | severity:functionality | blocker:no -->
...
<!-- hits:3 | last_caught:Loop-62 | class:cross-file-mismatch | severity:functionality | blocker:yes -->
...
<!-- hits:1 | last_caught:Loop-62 | class:security | severity:security | blocker:yes -->
...
<!-- hits:1 | last_caught:Loop-62 | class:mobile | severity:correctness | blocker:no -->
...
<!-- hits:1 | last_caught:Loop-62 | class:mobile | severity:craft | blocker:no -->
...
<!-- hits:2 | last_caught:Loop-62 | class:typescript | severity:correctness | blocker:yes -->
...
<!-- hits:2 | last_caught:Loop-62 | class:typescript | severity:craft | blocker:no -->
...
<!-- hits:5 | last_caught:Loop-60 | class:typescript | severity:correctness | blocker:no -->
...
<!-- hits:1 | last_caught:Loop-72 | class:analytics | severity:correctness | blocker:yes -->
...
<!-- hits:2 | last_caught:Loop-72 | class:state-management | severity:correctness | blocker:no -->
...
<!-- hits:1 | last_caught:Loop-72 | class:analytics | severity:correctness | blocker:no -->
...
<!-- hits:2 | last_caught:Loop-72 | class:typescript | severity:correctness | blocker:no -->
...
<!-- hits:2 | last_caught:Loop-72 | class:typescript | severity:craft | blocker:no -->
...
<!-- hits:2 | last_caught:Loop-72 | class:design-duplication | severity:craft | blocker:no -->
...
<!-- hits:7 | last_caught:Loop-72 | class:token-violation | severity:craft | blocker:no -->
...
<!-- hits:6 | last_caught:Loop-72 | class:token-violation | severity:craft | blocker:no -->
...
<!-- hits:5 | last_caught:Loop-72 | class:token-violation | severity:craft | blocker:no -->
...
<!-- hits:3 | last_caught:Loop-72 | class:state-management | severity:correctness | blocker:no -->
...
<!-- hits:1 | last_caught:Loop-72 | class:accessibility | severity:correctness | blocker:no -->
...
<!-- hits:2 | last_caught:Loop-72 | class:mobile | severity:functionality | blocker:no -->
...
<!-- hits:1 | last_caught:Loop-72 | class:accessibility | severity:craft | blocker:no -->
