  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <div className="absolute top-[15%] left-[20%] w-[400px] h-[400px] rounded-full opacity-20"
        style={{ background: 'radial-gradient(circle, #fcc824 0%, transparent 70%)', filter: 'blur(80px)', animation: 'orb-float-1 8s ease-in-out infinite' }} />
      <div className="absolute top-[30%] right-[15%] w-[350px] h-[350px] rounded-full opacity-15"
        style={{ background: 'radial-gradient(circle, #8b5cf6 0%, transparent 70%)', filter: 'blur(100px)', animation: 'orb-float-2 10s ease-in-out infinite' }} />
      <div className="absolute bottom-[10%] left-[40%] w-[300px] h-[300px] rounded-full opacity-10"
        style={{ background: 'radial-gradient(circle, #06b6d4 0%, transparent 70%)', filter: 'blur(90px)', animation: 'orb-float-3 12s ease-in-out infinite' }} />
    </div>
  );
}

/* ═══════════════════════════════════════════════
   VARIANT A — Sticky floating robot that stays
   on-screen as you scroll, with full-viewport
   mouse tracking via transparent overlay
   ═══════════════════════════════════════════════ */

export default function TrialV3A() {
  const [splineLoaded, setSplineLoaded] = useState(false);
  const [robotLoaded, setRobotLoaded] = useState(false);
  const [showRobot, setShowRobot] = useState(true);
  const splineRef = useRef<any>(null);

  /* Forward mouse events from the full-page overlay to the Spline canvas.
     Spline scenes with "Follow mouse" events on objects will react. */
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    if (!splineRef.current) return;
    try {
      // Spline's runtime emitEvent can trigger mouse-based interactions
  ...
  return (
    <div className="min-h-screen bg-[#07080f] text-white overflow-x-hidden">
      ...
      <style jsx global>{`
        ...
        @keyframes orb-float-3 { 0%, 100% { transform: translate(0,0) scale(1); } 50% { transform: translate(20px,-15px) scale(1.04); } }
      ...
      {/* ── FULL-PAGE MOUSE TRACKING OVERLAY ──
          This invisible div sits on top of the sticky robot so mouse events
          propagate through. The Spline canvas itself handles mouse tracking
          for the robot's look-at behavior. */}

      {/* ── STICKY FLOATING ROBOT (desktop only) ── */}
      {showRobot && (
        <div className="hidden lg:block fixed bottom-8 right-8 z-40 w-[280px] h-[320px]"
          style={{ animation: 'robot-bob 4s ease-in-out infinite' }}>
          {/* Glow */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[250px] h-[250px] bg-[#fcc824]/[0.06] rounded-full blur-[60px] pointer-events-none" />
          {/* Close button */}
          <button
            onClick={() => setShowRobot(false)}
            className="absolute -top-2 -right-2 z-50 w-7 h-7 rounded-full bg-white/10 border border-white/20 text-gray-400 hover:text-white hover:bg-white/20 transition-all flex items-center justify-center text-xs font-bold"
          >
            &times;
          </button>
          <Suspense fallback={
            <div className="w-full h-full flex items-center justify-center">
              <div className="w-16 h-16 rounded-full border-2 border-[#fcc824]/20 border-t-[#fcc824] animate-spin" />
            </div>
          }>
            <Spline
              scene={SPLINE_ROBOT_URL}
              onLoad={(splineApp) => { splineRef.current = splineApp; setRobotLoaded(true); }}
      ...
      <section className="relative min-h-[100vh] flex items-center">
        ...
        <div className="absolute inset-0 z-0">
          <Suspense fallback={null}>
            <Spline scene={SPLINE_SCENE_URL} onLoad={() => setSplineLoaded(true)}
              style={{ width: '100%', height: '100%', opacity: splineLoaded ? 0.85 : 0, transition: 'opacity 1.5s ease-in' }} />
          </Suspense>
        </div>
        ...
        {/* Hero text — full width since robot is sticky/floating */}
        <div className="relative z-[2] max-w-7xl mx-auto px-5 sm:px-8 pt-24">
          <div className="max-w-3xl">
            ...
            <p className="anim-hero-3 text-lg sm:text-xl text-gray-400 max-w-2xl leading-relaxed mb-10">
              ...
              and convert leads — all inside one system. Same frameworks used by 87+ mindset coaches earning $30k-$100k/month.
            </p>
        ...
        <div className="relative max-w-6xl mx-auto px-5 sm:px-8">
          <RevealSection>
            <div className="text-center mb-20">
              ...
              <p className="text-lg text-gray-500 max-w-2xl mx-auto leading-relaxed">14 specialized agents organized into 5 workflow stages. Each handles a different piece of building your mindset coaching business.</p>
            </div>
          </RevealSection>
            ...
            <div className="lg:col-span-3">
              ...
              <div className="space-y-4">
                {[
                  ...
                  { icon: TrendingUp, title: 'Velocity', text: 'Build offers that sell — based on frameworks proven across 87+ mindset coaches.' },
                  { icon: MessageSquare, title: 'AI Guidance', text: 'Agents that walk you through each step. Not just text generators.' },
                  { icon: Users, title: 'Community', text: 'Join mindset coaches already scaling with systematic, repeatable processes.' },
      ...
      <section className="relative py-28 sm:py-36 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-5 sm:px-8 text-center">
          <RevealSection>
            <h2 className="text-3xl sm:text-5xl lg:text-6xl font-black tracking-tight mb-6 leading-[1.1]">Stop building your business<br className="hidden sm:block" /><span className="text-[#fcc824]"> from scratch.</span></h2>
            <p className="text-lg sm:text-xl text-gray-400 mb-10 max-w-2xl mx-auto leading-relaxed">87+ mindset coaches already use MindsetOS to systematically build offers, run promotions, and convert clients. Your 7-day trial starts now.</p>
