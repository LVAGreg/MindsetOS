...
function registerChatRoutes(req, res, method, path, corsHeaders, dependencies) {
  ...
  if (path === '/api/letta/chat' && method === 'POST') {
    (async () => {
      ...
      // Check onboarding status and agent access
      try {
        ...
        const onboardingCompleted = onboardingCheck.rows.length > 0 && onboardingCheck.rows[0].onboarding_completed;

        // Check if agent requires onboarding to be completed
        const agentCheck = await pool.query(
          'SELECT locked_until_onboarding, requires_onboarding FROM agents WHERE id = $1',
          [agentId]
        );
        ...
        if (agentCheck.rows.length > 0) {
          ...
          // Block access if agent is locked and onboarding not completed
          if (agent.locked_until_onboarding && !onboardingCompleted) {
            res.writeHead(403, corsHeaders);
            res.end(JSON.stringify({
              error: 'Agent locked',
              message: 'Please complete onboarding to access this agent',
        ...
        // Continue - don't block on onboarding check errors
      }
